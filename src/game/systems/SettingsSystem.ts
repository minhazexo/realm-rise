...
import { AUTOSAVE_INTERVAL_SEC } from '../core/Constants.ts';
import type { GameSettings } from '../core/stateFactory.ts';
...
export const SETTINGS_DEFAULTS: GameSettings = Object.freeze({
  ...
  distanceFog: true,
  /** World camera zoom: 0.6 (wide) – 2 (close). Wheel / +/- / HUD buttons. */
  camZoom: 1,
  /** TEMPORARY god-mode for playtesting (user request): the player cannot
   *  die. Survival drains are paused and incoming damage is treated as a
   *  1 hp-tick of scratch damage. Flip back to false to restore mortality. */
  immortal: true,
  /** Player key bindings (KeyCodes names). See KEYBINDS_DEFAULT. */
  keybinds: { ...KEYBINDS_DEFAULT },
});

/** One scalar-setting clamp rule: a numeric range or an allowed set. */
...
const SCALAR_KEYS = [
  ...
  'distanceFog', 'immortal', 'fpsCap', 'camZoom'
];
 ...
 */
export function normaliseSettings(input: unknown): GameSettings {
  const out: GameSettings = deepClone(SETTINGS_DEFAULTS);
  if (!input || typeof input !== 'object') return out;
  const src = input as Record<string, unknown>;
  for (const k of SCALAR_KEYS) {
    if (src[k] === undefined) continue;
    out[k] = clampScalar(k, src[k]);
  }
  if (src.volumes && typeof src.volumes === 'object') {
    const vols = src.volumes as Record<string, unknown>;
    for (const vk of ['master', 'music', 'sfx']) {
      const v: number = Number(vols[vk]);
      if (Number.isFinite(v)) out.volumes[vk] = Math.max(0, Math.min(1, v));
    }
  }
  if (src.toggles && typeof src.toggles === 'object') {
    const tog = src.toggles as Record<string, unknown>;
    for (const tk of Object.keys(SETTINGS_DEFAULTS.toggles)) {
      if (typeof tog[tk] === 'boolean') out.toggles[tk] = tog[tk];
    }
  }
  if (src.keybinds && typeof src.keybinds === 'object') {
    const binds = src.keybinds as Record<string, unknown>;
    for (const bk of Object.keys(KEYBINDS_DEFAULT)) {
      if (validKeyCodeName(binds[bk])) out.keybinds[bk] = binds[bk];
    }
  }
  return out;
}
...
let _lastApplied: GameSettings | null = null;

/** Options for {@link applySettings}. */
export interface ApplySettingsOpts {
  /** Emit CH.SETTINGS / 'refresh-ui'. */
  busNotify?: boolean;
  /** Mirror to localStorage prefs. */
  persistPrefs?: boolean;
  /** Reapply audio bus gain values. */
  audio?: boolean;
}
 ...
 *
 * @param settings - already-normalised settings object
 * @param opts
 * @param opts.busNotify - emit CH.SETTINGS / 'refresh-ui'
 * @param opts.persistPrefs - mirror to localStorage prefs
 * @param opts.audio - reapply audio bus gain values
 */
export function applySettings(settings: GameSettings, opts: ApplySettingsOpts = {}): GameSettings {
  const { busNotify = true, persistPrefs: shouldPersist = true, audio = true } = opts;
  const s: GameSettings = normaliseSettings(settings);
  _lastApplied = s;
 ...
 *
 * NOTE: re-applies with busNotify:false — this handler is itself running
 * inside a CH.SETTINGS emission, so re-emitting would recurse until the
 * stack overflows (every updateSettings call would spam ~10k applications).
 */
export function installSettingsSystem(): void {
  if (_installed) return;
  _installed = true;
  Bus.on(CH.SETTINGS, () => {
    const s: GameSettings | undefined = GameState.s?.settings;
    if (s) applySettings(s, { busNotify: false });
  });
}
...
let _menuCache: GameSettings | null = null;
...
/** Partial settings patch accepted by {@link updateSettings}. */
 ...
 */
export function updateSettings(patch: SettingsPatch): GameSettings {
  let cur: GameSettings;
  ...
  const merged: GameSettings = { ...cur, ...patch };
  if (patch.volumes) merged.volumes = { ...cur.volumes, ...(patch.volumes as Record<string, unknown>) };
  if (patch.toggles) merged.toggles = { ...cur.toggles, ...(patch.toggles as Record<string, unknown>) };
  if (patch.keybinds) merged.keybinds = { ...cur.keybinds, ...(patch.keybinds as Record<string, unknown>) };
  const normalised: GameSettings = normaliseSettings(merged);
...
export function resetSettings(): GameSettings {
  const fresh: GameSettings = deepClone(SETTINGS_DEFAULTS);
...
export function clearMenuCache(): void { _menuCache = null; }
...
export function withPrefs(settings?: GameSettings | null): GameSettings {
  const prefs: Record<string, unknown> = loadPrefs();
  const merged: GameSettings = deepClone(settings || SETTINGS_DEFAULTS);
  ...
  if (prefs.volumes) merged.volumes = { ...merged.volumes, ...(prefs.volumes as Record<string, unknown>) };
  if (prefs.toggles) merged.toggles = { ...merged.toggles, ...(prefs.toggles as Record<string, unknown>) };
  return normaliseSettings(merged);
}

/** Read what was last applied (useful for HUD display + tests). */
export function lastApplied(): GameSettings | null { return _lastApplied ? deepClone(_lastApplied) : null; }

/* ── Live accessors used by other systems ───────────────────────────────── */

/** Cheap read of a single setting, falling back to default. */
export function getSetting(key: string): unknown {
  const live: unknown = GameState.s?.settings?.[key];
 ...
 */
export function photosensitiveMode(): boolean {
  const s: GameSettings | undefined = GameState.s?.settings;
  if (!s) return false;
...
export function particleMultiplier(): number {
  ...
  const p: string = GameState.s?.settings?.particles || 'high';
  return map[p] ?? 1;
}
...
export function movementKey(): string {
...
export function isImmortal(): boolean {
  return GameState.s?.settings?.immortal === true;
}
function clampScalar(key: string, raw: unknown): unknown {
  const lim: SettingsLimit | undefined = (SETTINGS_LIMITS as unknown as Record<string, SettingsLimit>)[key];
  if (!lim) return raw;
  if (lim.allowed) return lim.allowed.includes(raw as never) ? raw : SETTINGS_DEFAULTS[key];
...
function deepClone<T>(o: T): T {
  ...
  if (Array.isArray(o)) return o.map(deepClone) as T;
  const out: Record<string, unknown> = {};
  for (const k of Object.keys(o)) out[k] = deepClone((o as Record<string, unknown>)[k]);
  return out as T;
}
...
export function persistPrefs(settings: GameSettings): void {
  if (typeof localStorage === 'undefined') return;
