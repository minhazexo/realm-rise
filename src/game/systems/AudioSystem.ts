...
import { SFX_LIBRARY, setCtx } from './audioSfx.ts';
import type { SfxAudioContext } from './audioSfx.ts';
import { initAmbient, setAmbientVolume, stopAmbient, updateAmbient as _updateAmbient } from './ambientSounds.ts';

let ctx: SfxAudioContext | null = null;
...
/** Volume triple shared by the settings panel. */
...
function createAudioContext(): void {
  ...
  const stash = (typeof window !== 'undefined') ? window.__RISE_AUDIO__ : undefined;
  if (stash && stash.ctx) {
    const reused = stash.ctx as SfxAudioContext;
    ctx = reused;
    masterGain = stash.masterGain;
    musicGain = stash.musicGain;
    comp = stash.comp;
    limiter = stash.limiter;
    reused._sfxGain = stash.sfxGain;
    setCtx(reused);
    started = true;
    applyVolumes();
    return;
  ...
  if (!Ctor) return;
  const nc = new Ctor() as SfxAudioContext;
  ctx = nc;
  const mg = nc.createGain();
  const gmusic = nc.createGain();
  const sfx = nc.createGain();
  nc._sfxGain = sfx;
  masterGain = mg;
  musicGain = gmusic;
  ...
  const cm = nc.createDynamicsCompressor();
  cm.threshold.value = -18;
  cm.knee.value = 20;
  cm.ratio.value = 4;
  cm.attack.value = 0.004;
  cm.release.value = 0.16;
  const lim = nc.createDynamicsCompressor();
  ...
  mg.connect(cm);
  cm.connect(lim);
  lim.connect(nc.destination);
  setCtx(nc);
  initAmbient(nc, mg);
  applyVolumes();
  started = true;
  if (typeof window !== 'undefined') {
    window.__RISE_AUDIO__ = { ctx: nc, masterGain: mg, musicGain: gmusic, comp: cm, limiter: lim, sfxGain: sfx };
  }
}
...
export function initAudio(): boolean {
  ...
  const tryStartMenu = (): void => {
    if (!started) return;
    const c = ctx;
    if (c?.state === 'suspended') {
      c.resume().then(() => setMenuMood()).catch(() => setTimeout(tryStartMenu, 400));
    } else {
      setMenuMood();
    }
  };
...
export function applyVolumes(): void {
  ...
  const v: VolumePrefs = (GameState.s?.settings.volumes as VolumePrefs | undefined) || { master: 0.8, music: 0.55, sfx: 0.85 };
  const t: Record<string, unknown> = (GameState.s?.settings.toggles as Record<string, unknown> | undefined) || {};
  masterGain.gain.value = v.master * (t.musicOn === false && t.sfxOn === false ? 0 : 1);
  if (musicGain) musicGain.gain.value = t.musicOn === false ? 0 : v.music * 0.5;
  if (ctx) ctx._sfxGain.gain.value = t.sfxOn === false ? 0 : v.sfx;
...
export const hasSfx = (name: string): boolean => !!SFX_LIBRARY[name];

/* ── Generative mood music ─────────────────────────────────────────────── */
/** Apply a semitone offset to a frequency. */
const semiToHz = (freq: number, semi: number): number => freq * Math.pow(2, semi / 12);

function drumTick(t0: number): void {
  const c = ctx;
...
function padVoice(rootHz: number, semi: number, dur: number, peak: number, at = 0): void {
  const c = ctx;
  if (!c || !musicGain) return;
...
function bassVoice(rootHz: number, semi: number, dur: number, at = 0): void {
  const c = ctx;
  if (!c || !musicGain) return;
  const t0 = c.currentTime + at;
  const g = c.createGain();
...
function pluck(rootHz: number, semi: number, at: number, vel: number): void {
  const c = ctx;
  if (!c || !musicGain) return;
  const t0 = c.currentTime + at;
  const freq = semiToHz(rootHz, semi);
...
export function ensureMusicReverb(): void {
  const c = ctx;
  if (musicReverb || !c || !musicGain) return;
...
/** One chord bar in a mood progression. */
interface MoodBar {
  chord: number[];
  bass: number;
}

/** One melody note (offset in seconds from the phrase start). */
interface MoodNote {
  at: number;
  semi: number;
  vel: number;
...
/** Per-mood generative music configuration. */
interface MoodCfg {
  root: number;
  bar: number;
  pluckVel: number;
...
function moodPhraseStep(mood: string): void {
  const c = ctx;
  ...
  for (let b = 0; b < nbars; b++) {
    ...
    if (cfg.drums) {
      const beats = 4;
      for (let k = 0; k < beats; k++) drumTick(c.currentTime + at + k * (bar / beats));
    }
  }
...
const MUSIC_ENABLED: boolean = false;
...
export function setMood(mood: string): void {
  ...
  const c = ctx;
  if (c?.state === 'suspended') {
    c.resume().then(() => setMood(mood)).catch(() => {});
    return;
  }
  stopMusic();
...
export function setMenuMood(): void {
  setMood('menu');
}
...
export function stopAllAudio(): void {
  ...
  try { if (cc && cc.suspend) cc.suspend(); } catch { /* */ }
}
...
const AMBIENT_ENABLED: boolean = false;
...
if (import.meta.hot) {
  ...
  if (import.meta.hot.data && import.meta.hot.data.wasStarted) {
    const prevMood: string = import.meta.hot.data.mood || 'menu';
    setTimeout(() => {
      try {
        createAudioContext();               // reuses the single stashed context
        const c = ctx;
        if (c?.state === 'suspended') c.resume().then(() => setMood(prevMood)).catch(() => {});
