...
import { xpForLevel, MAX_PLAYER_LEVEL } from '../core/Constants.ts';
...
export const xpToNext = (level: number = st().player.level): number => xpForLevel(level);

/** Award experience. Returns true when at least one level was gained. */
export function awardXP(baseAmount: number, sourceLabel = ''): boolean {
  const P = st().player;
  if (!P || P.level >= MAX_PLAYER_LEVEL) return false;
  P.xp += Math.max(0, Math.round(baseAmount * (P.derived?.xpGainMult ?? 1)));
  let leveled = false;
  while (P.level < MAX_PLAYER_LEVEL && P.xp >= xpToNext(P.level)) {
    ...
    import('./ProgressionSystem.ts')
      .then((m) => m.recompute())
      .catch(() => {});
    const D = P.derived;
    if (D) P.hp = Math.min(D.maxHp + 30, P.hp + D.maxHp * 0.3);
    Bus.emit('level-up', { level: P.level, source: sourceLabel });
        import('./AchievementSystem.ts').then((m) => m.checkLevel(P.level)).catch(() => {});
  }
  if (!leveled) GameState.notify(CH.PLAYER);
  return leveled;
}

export function spendStat(key: string): boolean {
  const P = st().player;
  if (P.statPoints <= 0) return false;
  const cur: number | undefined = P.alloc[key];
  if (cur === undefined || cur >= 30) return false;
  P.statPoints--;
  P.alloc[key] = cur + 1;
  GameState.notify(CH.PLAYER);
  return true;
...
export function learnSkill(id: string): boolean {
  const P = st().player;
  const def = SKILL_MAP[id];
  if (!def || P.skillPoints <= 0) return false;
  const rank: number = P.skills[id] || 0;
...
const profNeed = (lv: number): number => Math.round(70 + Math.pow(lv, 1.65) * 52);

export function profXP(prof: string, amount: number): number {
...
export const profLevel = (prof: string): number => st().player.professions[prof]?.lv ?? 0;

/* ── Reputation ────────────────────────────────────────────────────────── */
export function addReputation(n: number): number {
  const P = st().player;
  let val: number = n * (1 + (P.derived?.willpower || 0) * 0.01);
...
export function healToFull(): void {
  const S = st();
  if (!S) return;
  S.player.hp = S.player.derived?.maxHp ?? S.player.hp;
  S.player.stamina = S.player.derived?.maxStamina ?? S.player.stamina;
  GameState.notify(CH.PLAYER);
}

const cap = (s: string): string => s.charAt(0).toUpperCase() + s.slice(1);
const roman = (lv: number): string => (['', 'I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X'][Math.min(lv, 10)] ?? '');
