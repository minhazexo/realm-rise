...
/** One recycled fog card in the scene-local pool. */
interface FogCard {
...
/** Minimal scene surface consumed by the fog-card pool. */
export type FogScene = Phaser.Scene & {
  player?: { sprite?: { x: number; y: number } } | null;
  env?: { weather?: string } | null;
  _fogCards?: FogCard[];
};

/** Desired card count from weather + biome + time. */
function wantedCount(weather: string, biome: string, isNight: boolean): number {
  let n = 0;
  if (weather === 'fog') n += 8;
...
function windFor(weather: string): WindVec {
  switch (weather) {
...
export function updateFogCards(scene: FogScene, dt: number): void {
  const S = GameState.s;
  ...
  if (!scene._fogCards) {
    ...
    for (let i = 0; i < POOL; i++) {
      const img: Phaser.GameObjects.Image = scene.add.image(p.x, p.y, 'pt_smoke')
        .setDepth(60)
        .setAlpha(0)
  ...
  const t: number = S.world?.timeOfDay ?? 0.5;
  const isNight = t > 0.78 || t < 0.24;
  const weather: string = S.world?.activeWeather || scene.env?.weather || 'clear';
  let biome = 'plains';
  try { biome = biomeAt(p.x, p.y); } catch { /* default */ }
  const want: number = Math.round(wantedCount(weather, biome, isNight) * particleMultiplier());
  const wind: WindVec = windFor(weather);
