...
/** A light that should exist right now (world position + radius scale). */
...
/** Live light entry owned by the scene. */
interface DynLightEntry {
  sprite: Phaser.GameObjects.Image;
  def: WantedLight;
  phase: number;
}

/** Minimal scene surface consumed by the dynamic-light layer. */
export type DynLightScene = Phaser.Scene & {
  _dynLights?: Map<string, DynLightEntry>;
};

/** Bake a softer, bigger radial gradient than the existing fx_light. */
export function ensureDynamicLightTexture(scene: Phaser.Scene): void {
  if (_ready) return;
 ...
 */
export function refreshDynamicLights(scene: DynLightScene): void {
  ensureDynamicLightTexture(scene);
  if (!scene._dynLights) scene._dynLights = new Map<string, DynLightEntry>();
  const dynLights = scene._dynLights;

  const wanted: Map<string, WantedLight> = collectWantedLights();

  // Remove lights that no longer exist
  for (const [key, light] of dynLights) {
    if (!wanted.has(key)) {
      light.sprite.destroy();
      dynLights.delete(key);
    }
  }
  ...
  let dayDim = 1;
  try {
    const t: number | undefined = GameState.s?.world?.timeOfDay;
    if (typeof t === 'number') {
  ...
  for (const [key, def] of wanted) {
    let entry = dynLights.get(key);
    if (!entry) {
      const tex: string = scene.textures.exists(TEX_KEY) ? TEX_KEY : 'fx_light';
      const sprite: Phaser.GameObjects.Image = scene.add.image(def.x, def.y, tex)
        .setBlendMode('ADD')
        .setDepth(3850)
        .setScrollFactor(1)
        .setScale(def.scale);
      entry = { sprite, def, phase: Math.random() * Math.PI * 2 };
      dynLights.set(key, entry);
    } else {
...
export function collectWantedLights(): Map<string, WantedLight> {
  const out = new Map<string, WantedLight>();
  const S = GameState.s;
  if (!S) return out;
  ...
  const offhandId: string | undefined = S.player.equipment?.offhand?.id;
...
Bus.on('player-pos', (xy: unknown) => {
  const p = xy as { x?: unknown; y?: unknown } | null;
  if (p && typeof p.x === 'number') {
    GameState._playerWorldXY.x = p.x;
    GameState._playerWorldXY.y = Number(p.y) || 0;
  }
});
