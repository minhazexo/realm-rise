...
// ─────────────────────────────────────────────────────────────────────────────
import { single, circ, ell, rr, shade, seededRandom, resetRandom } from './artCore.ts';
 ...
 */
export function buildFxProps(scene: Phaser.Scene): void {
  // ── Shadows & lighting ────────────────────────────────────────────────
 ...
 * @param r  Overall diameter hint.
 */
function crescent(ctx: CanvasRenderingContext2D, r: number): void {
  const cx = r / 2;
  const cy = r / 2;
  // bright core
 ...
 */
export function buildMenuProps(scene: Phaser.Scene): void {
  ...
  single(scene, 'menu_cloud', 260, 100, (ctx) => {
    ...
    ([
      ...
      [180, 52, 40], [100, 68, 28],
      [155, 65, 24], [50, 48, 22],
    ] as const).forEach(([x, y, r]) => puff(x, y, r, 0.75));
  });
   ...
   * @param peakH  Maximum hill height above the baseline.
   */
  const hillLayer = (key: string, color: string, peakH: number): void =>
  ...
  single(scene, 'menu_castle', 380, 280, (ctx, w, h) => {
    ...
    const tower = (x: number, tw: number, th: number, roofH: number = 28): void => {
 ...
 * @param dir  -1 = wings up, +1 = wings down.
 */
function birdWing(ctx: CanvasRenderingContext2D, dir: number): void {
