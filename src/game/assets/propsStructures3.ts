...
// ─────────────────────────────────────────────────────────────────────────────
import { single, rr, circ, ell, tri, shade, O } from './artCore.ts';
import { structure } from './propsStructures1.ts';
import type * as Phaser from 'phaser';

/**
 * Build defense, civic, and decorative structure textures.
 */
export function buildStructureProps3(scene: Phaser.Scene): void {
  // ── Wall Segment ──────────────────────────────────────────────────────
  ...
  single(scene, 'market_stalls', 84, 62, (ctx, w, h) => {
    ...
    const canopies: Array<[string, number]> = [['#e05050', 6], ['#e8c94b', 44]];
    canopies.forEach(([c, x]) => {
      // posts
  ...
  // ── Chest Variants ────────────────────────────────────────────────────
  // [key, main colour]
  const chests: Array<[string, string]> = [
    ['wooden_chest', '#937249'],
    ['iron_chest', '#aeb4bd'],
    ['royal_chest', '#c9a24b'],
    ['ancient_chest', '#57c4ab'],
  ];
  chests.forEach(([key, c]) => {
