...
import { O, blade, crossguard, hilt, handle, headTri, tri2, diamond } from './iconPrims.ts';
import type { IconDrawer } from './iconPrims.ts';

/** Weapon / armour icon drawers keyed by shape name. */
export const SHAPES_A: Record<string, IconDrawer> = {
  /**
   * Sword — blade + crossguard + hilt.
   * @param c  Blade colour.
   */
  sword(ctx, c) {
    blade(ctx, c);
    crossguard(ctx);
