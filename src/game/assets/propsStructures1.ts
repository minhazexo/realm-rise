...
// ─────────────────────────────────────────────────────────────────────────────
import { single, rr, circ, ell, tri, O, shade } from './artCore.ts';
import type * as Phaser from 'phaser';

// ── Shared structure helper ─────────────────────────────────────────────────

/** Options for the shared {@link structure} building helper. */
export interface StructureOpts {
  ...
  /** Door / trim colour. */
  trim?: string;
  /** Width multiplier (>1 = wider building). */
  wide?: number;
  /**
   * Upgrade tier (0 = basic … 4 = endgame).
   *   - Tier ≥ 2: stone texture lines on walls.
   *   - Tier ≥ 3: gold chimney / banner.
   */
  tier?: number;
  /** 'gable' (triangle) or 'flat' (rect). */
  roof?: string;
}
 ...
 * @param opts
 */
export function structure(ctx: CanvasRenderingContext2D, w: number, h: number, opts: StructureOpts): void {
  const {
    wallC  = '#93683e',
    roofC  = '#7a4e31',
    trim   = '#6b4a2c',
 ...
 */
export function buildStructureProps1(scene: Phaser.Scene): void {
  // ── Campfire ──────────────────────────────────────────────────────────
  single(scene, 'campfire', 34, 30, (ctx, w, h) => {
    ell(ctx, w / 2, h - 4, 12, 4, 'rgba(0,0,0,0.15)');
    // logs: [offsetX, offsetY]
    ([[-8, -6], [8, -4], [-5, -1], [6, -8]] as const).forEach(([dx, dy]) =>
  ...
  hallPals.forEach((pal, i) => {
    single(scene, `townhall_t${i + 1}`, 96 + i * 6, 92 + i * 6, (ctx, w, h) => {
      ...
      for (let t = 0; t < towers; t++) {
        ...
        rr(ctx, tx, h - 54 - i * 4, 13, 48 + i * 4, 2, pal.wallC ?? '#93683e', O);
        tri(ctx, tx - 3, h - 66 - i * 4, 19, pal.roofC ?? '#7a4e31', O);
        // pennant
