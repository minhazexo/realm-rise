...
import type { ItemMods } from './itemDefs.ts';

type ArmorSlot = 'helmet' | 'chest' | 'gloves' | 'boots';

interface ArmorDefOpts {
  rarity?: string;
  value?: number;
  mods?: ItemMods;
  c1?: string;
...
const armorDef = (id: string, name: string, slot: ArmorSlot, armor: number, tier: number, warmth: number, opts: ArmorDefOpts = {}): void => {
  void name;
  def(id, {
    cat: 'armor',
    slot,
