...
// Strict skill registry: a duplicated skill id used to collapse silently in
// Object.fromEntries. Order-preserving: SKILL_MAP keeps authoring order.
const SKILL_REGISTRY = new Registry<SkillDef>('skill', { required: ['name', 'branch', 'maxRank'] });
for (const s of SKILLS) SKILL_REGISTRY.define(s.id, s);
SKILL_REGISTRY.seal();

export const SKILL_MAP: Record<string, SkillDef> = SKILL_REGISTRY.snapshot();
