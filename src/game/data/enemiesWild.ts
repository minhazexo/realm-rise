...
import type { EnemyTeam } from './enemyTeams.ts';
...
export interface EnemyPhaseSummon {
  type: string;
  count: number;
}
...
export interface EnemyPhase {
  belowHp: number;
  moves: string[];
  summon?: EnemyPhaseSummon;
  enrageSpeed?: number;
  beamDpsTick?: number;
}

export type EnemyStyle = 'kiter' | 'melee' | 'brute' | 'boss' | 'pounce' | 'charger';

export type EnemyPalette = Record<string, string | boolean>;
