...
export type BuildingCostMap = Record<string, number>;

export type BuildingEffects = Record<string, boolean | number | string[]>;

export interface BuildingTowerAttack {
  range: number;
  dmg: [number, number];
  cdSec: number;
}
...
export interface BuildingDef {
  ...
  tierProduceBonus?: Record<string, number>;
  tierCosts?: (BuildingCostMap | null)[];
  tierLabels?: string[];
  tierDefense?: number[];
  defenseByTier?: number[];
  towerAttack?: BuildingTowerAttack;
  hpByTier?: number[];
  hp?: number;
  taxPerDay?: number[];
  tradePriceBonus?: number[];
  buildAura?: number;
  yieldMultNearMountain?: number;
}

export const BUILDINGS_A: Record<string, BuildingDef> = {
  campfire: {
