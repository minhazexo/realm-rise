...
export type FactionPerks = Record<string, number | boolean>;

export type FactionLikes = Record<string, boolean | string>;
...
export const relLabel = (v: number): string => REL_STAGES.find((s) => v >= s.min)?.label || 'Neutral';

const FACTION_REGISTRY = new Registry<FactionDef>('faction', { required: ['name'] });
FACTION_REGISTRY.defineAll(FACTIONS);
FACTION_REGISTRY.seal();

export const getFactionDef = (key: string): FactionDef | null => FACTION_REGISTRY.get(key);
