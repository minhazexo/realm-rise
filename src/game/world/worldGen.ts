...
/** Current world seed (unsigned 32-bit). */
let WORLD_SEED: number = 1;
 ...
 */
export const RIVER_LEVEL: number = 0.34;
 ...
 * @returns Elevation value in [0, 1).
 */
 ...
 * @param wx  World X in pixels.
 * @param wy  World Y in pixels.
 * @returns Approximate temperature in [-1, 1] range.
 */
export function tempAt(wx: number, wy: number): number {
 ...
 * @param wx  World X in pixels.
 * @param wy  World Y in pixels.
 * @returns Biome key (e.g. `'plains'`, `'forest'`, `'mountains'`).
 */
export function biomeAt(wx: number, wy: number): string {
 ...
 * @param wy  World Y in pixels.
 */
export function isWaterAt(wx: number, wy: number): boolean {
  return elevationAt(wx, wy) < RIVER_LEVEL - 0.06;
}

// ── Points of Interest ──────────────────────────────────────────────────────

/** Deterministic point-of-interest descriptor. */
...
export function proceduralPois(): PointOfInterest[] {
  ...
  const push = (o: Omit<PointOfInterest, 'discovered' | 'looted'>): void => {
    spots.push({ discovered: false, looted: false, ...o });
  };
...
export const TERRITORY_TARGET: number = 18;
 ...
 *
 * @param nearX  Anchor world X.
 * @param nearY  Anchor world Y.
 * @returns Deterministic cache position nearby.
 */
export function treasureCacheSpot(nearX: number, nearY: number): { x: number; y: number } {
