...
/** Weighted resource spawn entry. */
export interface BiomeResourceEntry {
  type: string;
  weight: number;
}

/** Weighted enemy spawn entry. */
...
export interface BiomeEntry {
  ...
  /** Display name. */
  label: string;
  /** Primary grass colour hex. */
  grass: string;
  /** Darker grass variant hex (vignette / blotches). */
  grassDark: string;
  /** Accent colour hex (foliage, highlights). */
  accent: string;
  /** Decoration density multiplier (0-1). */
  decoDensity: number;
  /** Music mood key for the audio system. */
  musicMood?: string;
  /** Resource spawn table. */
  resources: BiomeResourceEntry[];
  /** Enemy spawn table. */
  enemies: BiomeEnemyEntry[];
  /** Particle effect keys. */
  ambient: string[];
  /** Temperature offset applied to tempAt(). */
  tempBias: number;
  /** Near-water biome — draws shore transition. */
  waterEdge?: boolean;
  /** Water uses green-brown palette instead of blue. */
  murkyWater?: boolean;
  /** Rocky terrain — grey accent speckles. */
  rocky?: boolean;
  /** Sandy terrain — tan/gold speckles. */
  sandy?: boolean;
  /** Snow terrain — white speckles + snow drifts. */
  snowy?: boolean;
  /** Volcanic ashen — orange/grey speckles. */
  ashen?: boolean;
  /** Glowing ember particles on ground. */
  emberSpecks?: boolean;
  /** Heat shimmer visual effect. */
  heatWave?: boolean;
  /** Frost / cold visual effect. */
  coldWave?: boolean;
  /** Enemy difficulty multiplier (default 1). */
  dangerMult?: number;
}

export const BIOMES: Record<string, BiomeEntry> = {
  // ── Temperate biomes ────────────────────────────────────────────────────
