...
export const PROFESSIONS: string[] = ['woodcutting', 'mining', 'survival', 'combat', 'crafting'];
...
export const xpForLevel = (level: number): number => Math.round(58 + Math.pow(level, 1.62) * 17);
...
export type DifficultyKey = 'story' | 'normal' | 'hard' | 'legendary';
...
export const DIFFICULTY: Readonly<Record<DifficultyKey, DifficultyDef>> = Object.freeze({
...
export interface GatherConfig {
  ticksPerNode: number; // hits before node breaks
  tickTime: number;
  tickXp: number; // XP per finished gather (Phase E: single source of truth)
  yieldBonusPerProfessionLevel: number;
  toolTierSpeeds: number[]; // none/st iron/steel handled per tool def
  regrowTimeMin: number;
  regrowTimeVar: number;
}

export const GATHER_CONFIG: GatherConfig = {
...
export interface SettlementStageReq {
...
export interface SettlementConfig {
  claimRadius: number; // radius around town hall considered home territory
  stageNames: string[];
  /** Requirements to REACH next stage index (all must pass). */
  stages: (SettlementStageReq | null)[];
  productionTickSec: number;
}
...
export const BUILD_RADIUS_FROM_HALL: number[] = [420, 720, 1080, 1400, 1700, 2000, 2300];
...
export interface MilitaryConfig {
  types: Record<string, MilitaryUnitDef>;
  moraleFromHappiness: (h: number) => number;
}

export const MILITARY_CONFIG: MilitaryConfig = {
  types: {
  ...
  moraleFromHappiness: (h: number): number => 0.7 + h / 166
};
