...
export default function DebugPanel({ visible }: DebugPanelProps): JSX.Element | null {
  ...
  const btn = (label: string, fn: () => void): JSX.Element => (
    ...
    <div className="debug-panel active" style={{ maxHeight: 400, overflow: 'auto' }}>
      ...
      <div style={{ margin: '4px 0', display: 'flex', flexWrap: 'wrap', gap: 2 }}>
        {grid.map((g) => btn(g.icon + ' ' + g.label, () => {
                    import('../../game/systems/InventorySystem.ts').then((m: any) => m.addItem(g.label, 50));
        }))}
                {btn('XP+1k', () => {
                  import('../../game/systems/ProgressionXP.ts').then((m: any) => m.awardXP(1000, 'debug'));
                })}
        {btn('Gold+500', () => {
                    import('../../game/systems/ProgressionXP.ts').then((m: any) => m.addGold(500));
        })}
        {btn('Heal', () => {
                    import('../../game/systems/ProgressionXP.ts').then((m: any) => m.healToFull());
        })}
        {btn('Lvl+5', () => {
          for (let i = 0; i < 5; i++) {
            import('../../game/systems/ProgressionXP.ts').then((m: any) => m.awardXP(9999, 'debug'));
          }
        })}
      </div>
    </div>
