...
import { formatNum } from '../../utils/math.ts';
import { DIFFICULTY } from '../../game/core/Constants.ts';
...
const select = (): HudSnapshot => {
  const S: any = GameState.s;
  const D: any = S?.player?.derived;
  return {
...
export default function HUD(): JSX.Element | null {
  const h = useGameState([CH.PLAYER, CH.INVENTORY, CH.SETTLEMENT, CH.TIME, CH.WEATHER, CH.SCREEN], select) as HudSnapshot | null;
  if (!h || h.hp == null) return null;

  const pct = (v: number, m: number): number => Math.max(0, Math.min(100, (v / (m || 1)) * 100));
  const stageName: string | undefined = ['Wanderer','Camp','Village','Town','City','Kingdom','Empire'][Math.max(0, h.stage + 1)];
    ...
    <div className="hud-root">
      ...
      <div className="hud-bottom">
        <div className="hotbar">
          {['I','C','M','J','K','B','P'].map(([key]: string, _i: number): null => null)}
          {[
        ...
        <div className="hud-footer">
          <span className="diff-pill">{DIFFICULTY[h.difficulty as keyof typeof DIFFICULTY]?.label}</span>
...
function xpNeed(lv: number): number {
...
function weatherIcon(w: string): string {
  const icons: Record<string, string> = { clear: '☀', drizzle: '🌦', storm: '⛈', fog: '🌫', snow: '❄', heat: '☀🔥' };
  return icons[w] || '☁';
}
